...
namespace Hors.Tests
{
    public partial class Tests
    {
        ...
        [Test]
        public void TestCreateTextWithManyDates()
        {
            var parser = new HorsTextParser();
            var result = parser.Parse(
                "Завтра в 9 перекинуть 10 рублей",
                new DateTime(2019, 10, 13), 3
            );
            
            Assert.AreEqual(2, result.Dates.Count);
            var date = result.Dates.First();
            var text = result.CreateText(0);
            Assert.AreEqual(9, date.DateFrom.Hour);
            Assert.AreEqual("перекинуть 10 рублей", text);
        }
        
        [Test]
        public void TestCreateTextWithEndDate()
        {
            var parser = new HorsTextParser();
            var result = parser.Parse(
                "12.50 завтра в 9",
            ...
            Assert.AreEqual(2, result.Dates.Count);
            var date = result.Dates.First();
            var text = result.CreateText(1);
            Assert.AreEqual(9, date.DateFrom.Hour);
            Assert.AreEqual("12.50", text);
        }
        
        [Test]
        public void TestCreateTextWithManyDatesReverse()
        {
            var parser = new HorsTextParser();
            var result = parser.Parse(
                "Перекинуть 10 рублей завтра в 9",
                new DateTime(2019, 10, 13), 3
            );
            
            Assert.AreEqual(2, result.Dates.Count);
            var date = result.Dates.Last();
            var text = result.CreateText(1);
            Assert.AreEqual(9, date.DateFrom.Hour);
            Assert.AreEqual("Перекинуть 10 рублей", text);
        }
