...
namespace Hors
{
    public class HorsTextParser
    {
        ...
        public HorsParseResult Parse(string text, DateTime userDate, int collapseDistance = 4, bool ignoreSingleTokens = false)
        {
            ...
            // if only 1 token, return result as is
            if (tokens.Length == 1 && !ignoreSingleTokens)
            {
                return new HorsParseResult(text, tokens.ToList(), new List<DateTimeToken>(0));
            }
